...
public class DrugSeedGiveCommand extends CustomCommand
{
    private Main main;
    private ConfigManager configManager;
    
    public DrugSeedGiveCommand(final Main main) {
        super("dr", new String[] { "seed", "give" }, "Used to give a player a configured drug seed.", "/dr seed give <player> <drug> <amount>", "ultimatedrugs.seed.give", 2, 3);
        this.main = main;
    ...
    public void executeCommand(final CommandSender commandSender, final List<String> list) {
        ...
        if (player != null) {
            ...
            if (drugById != null) {
                if (drugById.getGrowing() != null) {
                    final ItemStack clone = drugById.getGrowing().getSeedItemStack().clone();
                    if (list.size() >= 3) {
                    ...
                    if (player.getInventory().firstEmpty() != -1) {
                        ...
                        commandSender.sendMessage(TextUtils.replaceVariables(this.configManager.getLangString("seed-give-successful"), hashMap));
                    }
                    else {
                    ...
                    commandSender.sendMessage(this.configManager.getLangString("drug-has-no-seed"));
                }
            }
            else {
